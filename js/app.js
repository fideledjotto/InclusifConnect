(() => {
  ...
  const DOCS = [
    ...
    { id: 7, title: "Créer des documents accessibles avec Word", type: "Guide", cat: "numerique", author: "Équipe Inclusif Connect", date: "2026-07-20", pop: 88, kw: ["bureautique", "word", "numérique"], url: "documents/word-accessible.pdf", img: "images/word.jpg" },
    ...
    { id: 11, title: "Vos droits face à l'employeur : synthèse juridique", type: "Document", cat: "loi", author: "Défenseur des droits", date: "2026-04-02", pop: 112, kw: ["emploi", "droit", "loi"], url: "documents/droits-emploi.pdf", img: "images/categories/theme-loi.jpg" },
    { id: 12, title: "Aides techniques pour la mobilité réduite", type: "Guide", cat: "moteur", author: "CNSA", date: "2026-03-08", pop: 47, url: "documents/aides-mobilite.pdf", img: "images/categories/theme-moteur.jpg" }
  ];
  ...
  const API_BASE = (window.location.origin && window.location.origin !== "null") ? window.location.origin : "http://127.0.0.1:8000";
  ...
  async function apiJson(url, options = {}) {
    ...
    if (!response.ok) {
      const message = data.errors ? Object.values(data.errors).flat().join(" ") : (data.message || "Erreur de communication avec le serveur.");
  ...
  function getStoredAuth() {
    try {
      const token = localStorage.getItem("ic-auth-token");
      const user = localStorage.getItem("ic-user");
      return {
        token: token || "",
  ...
  function getInitials(value = "Utilisateur") {
    return value
      .split(/\s+/)
      .filter(Boolean)
      .slice(0, 2)
  ...
  function buildAvatarDataUrl(name = "Utilisateur") {
      ...
      <svg xmlns="http://www.w3.org/2000/svg" width="120" height="120" viewBox="0 0 120 120">
        <defs>
          <linearGradient id="g" x1="0" x2="1" y1="0" y2="1">
            ...
            <stop offset="100%" stop-color="#4a2d8f"/>
          </linearGradient>
        </defs>
        <rect width="120" height="120" rx="60" fill="url(#g)"/>
        <circle cx="60" cy="42" r="20" fill="rgba(255,255,255,0.22)"/>
        <path d="M28 96c7-18 22-28 32-28s25 10 32 28" fill="rgba(255,255,255,0.22)"/>
        <text x="60" y="68" text-anchor="middle" font-size="32" font-family="Arial, sans-serif" font-weight="700" fill="#ffffff">${initials}</text>
      </svg>
    `;
  ...
  function getUserAvatar() {
    const customAvatar = localStorage.getItem("ic-user-avatar");
    if (customAvatar) return customAvatar;
    return buildAvatarDataUrl(getDisplayName());
  }

  function getNotifications() {
    try {
      const value = localStorage.getItem("ic-notifications");
  ...
  function addNotification(message) {
    ...
    localStorage.setItem("ic-notifications", JSON.stringify(notifications.slice(0, 6)));
  }

  function getDownloadHistory() {
    try {
      const value = localStorage.getItem("ic-download-history");
      return value ? JSON.parse(value) : [];
    } catch {
      return [];
    }
  ...
  function recordDownload(doc) {
    ...
    localStorage.setItem("ic-download-history", JSON.stringify(history.slice(0, 8)));
    addNotification(`Téléchargement enregistré : ${doc.title}`);
  }
  ...
  function applyAuthState() {
    ...
    accountBtn.setAttribute("aria-label", auth.token && auth.user ? "Mon compte" : "Créer un compte");
    accountBtn.title = auth.token && auth.user ? "Mon compte" : "Créer un compte";
    ...
    if (auth.token && auth.user) {
      ...
      accountBtn.innerHTML = `
        <span class="profile-avatar-shell" aria-hidden="true">
          <img src="${avatarUrl}" alt="Photo de profil" class="profile-avatar-image">
        </span>
      `;
      if (footerLogin) footerLogin.textContent = "Mon compte";
    } else {
      accountBtn.classList.remove("profile-button");
      accountBtn.classList.remove("profile-button--guest");
  ...
  function showAccountPanel() {
    ...
    openModal(`
      <div class="profile-panel">
        <div class="profile-header-card">
          <button type="button" class="profile-trigger" id="profileAvatarTrigger" aria-label="Changer la photo de profil">
            <img src="${getUserAvatar()}" alt="Photo de profil" class="profile-avatar-image profile-avatar-image--large">
          </button>
          <div>
            <h2 id="modalTitle" style="margin:0;">Mon compte</h2>
            <p style="margin:6px 0 0;color:var(--ink-soft);">${escapeHtml(auth.user.name || "Utilisateur")}</p>
          </div>
        </div>
        ...
        <div class="profile-grid-two">
          <section class="profile-section">
            ...
            <ul class="profile-list">
              ${notifications.length
                ? notifications.map((n) => `<li><span class="dot"></span><div><strong>${escapeHtml(n.message)}</strong><small>${new Date(n.date).toLocaleDateString("fr-FR", { day: "numeric", month: "short" })}</small></div></li>`).join("")
                : '<li><span class="dot"></span><div><strong>Aucune notification</strong><small>Vous êtes à jour.</small></div></li>'}
            </ul>
          </section>
          ...
          <section class="profile-section">
            ...
            <ul class="profile-list">
              ${downloads.length
                ? downloads.map((d) => `<li><span class="dot dot--gold"></span><div><strong>${escapeHtml(d.title)}</strong><small>${escapeHtml(d.type)} · ${new Date(d.date).toLocaleDateString("fr-FR", { day: "numeric", month: "short" })}</small></div></li>`).join("")
                : '<li><span class="dot dot--gold"></span><div><strong>Aucun téléchargement</strong><small>Vos documents apparaîtront ici.</small></div></li>'}
            </ul>
          </section>
        </div>

        <div class="profile-actions">
          <label class="btn btn-ghost profile-upload" for="avatarUpload">Choisir une photo</label>
          <input id="avatarUpload" type="file" accept="image/*" hidden>
          <button type="button" class="btn btn-primary" id="btnLogout">Se déconnecter</button>
        </div>
      </div>
    `);
    ...
    if (profileAvatarTrigger) {
      profileAvatarTrigger.addEventListener("click", () => avatarUpload?.click());
    }
    ...
    if (avatarUpload) {
      avatarUpload.addEventListener("change", (event) => {
        ...
        reader.onload = () => {
          localStorage.setItem("ic-user-avatar", String(reader.result));
          applyAuthState();
          showAccountPanel();
          announce("Photo de profil mise à jour.");
        };
    ...
    if (logoutBtn) {
      logoutBtn.addEventListener("click", () => {
        localStorage.removeItem("ic-auth-token");
        localStorage.removeItem("ic-user");
        localStorage.removeItem("ic-user-avatar");
        applyAuthState();
        closeModal();
  ...
  function openLoginModal(prefillEmail = "") {
    openModal(`
      <h2 id="modalTitle">Se connecter</h2>
      <p style="color:var(--ink-soft);">Accédez à votre espace Inclusif Connect.</p>
      <form class="account-form" id="loginForm">
        <label>Adresse e-mail<input type="email" name="email" value="${escapeHtml(prefillEmail)}" required></label>
        <label>Mot de passe<input type="password" name="password" required minlength="8"></label>
        <div class="form-message" hidden role="alert" style="color:#a82828;font-size:0.85rem;font-weight:600;"></div>
        <button class="btn btn-primary" type="submit" style="justify-content:center;">Se connecter</button>
      </form>
      <p style="margin-top:16px;color:var(--ink-soft);font-size:0.95rem;">
        Pas encore de compte ?
        <button type="button" class="link-btn" data-open-signup="1" style="background:none;border:none;padding:0;color:var(--violet-600);font-weight:700;cursor:pointer;">Créer un compte</button>
      </p>`);

    bindLoginForm(document.getElementById("loginForm"));

    const signupLink = document.querySelector("[data-open-signup]");
    if (signupLink) {
      signupLink.addEventListener("click", () => openAccountModal(document.getElementById("loginForm").elements.email.value.trim()));
    }
  }

  function bindLoginForm(form) {
    const errorBox = form.querySelector(".form-message");
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (data.token) localStorage.setItem("ic-auth-token", data.token);
        if (data.user) localStorage.setItem("ic-user", JSON.stringify(data.user));

        applyAuthState();
        announce("Connexion réussie. Bienvenue sur Inclusif Connect.");
        form.reset();
        closeModal();
        setTimeout(() => showAccountPanel(), 150);
      } catch (error) {
        errorBox.textContent = error.message || "Impossible de se connecter pour le moment.";
        errorBox.hidden = false;
        announce(error.message || "Impossible de se connecter pour le moment.");
      }
    });
  }

  function openAccountModal(prefillEmail = "") {
    openModal(`
      ...
      <form class="account-form" id="accountForm">
        ...
        <label>Adresse e-mail<input type="email" name="email" value="${escapeHtml(prefillEmail)}" required></label>
        <label>Mot de passe<input type="password" name="password" required minlength="8"></label>
        <label>Confirmer le mot de passe<input type="password" name="password_confirmation" required minlength="8"></label>
        <div class="form-message" hidden role="alert" style="color:#a82828;font-size:0.85rem;font-weight:600;"></div>
        <button class="btn btn-primary" type="submit" style="justify-content:center;">Créer mon compte</button>
      </form>
      <p style="margin-top:16px;color:var(--ink-soft);font-size:0.95rem;">
        Vous avez déjà un compte ?
        <button type="button" class="link-btn" data-open-login="1" style="background:none;border:none;padding:0;color:var(--violet-600);font-weight:700;cursor:pointer;">Se connecter</button>
      </p>`);

    bindAccountForm(document.getElementById("accountForm"));

    const loginLink = document.querySelector("[data-open-login]");
    if (loginLink) {
      loginLink.addEventListener("click", () => openLoginModal(document.getElementById("accountForm").elements.email.value.trim()));
    }
  }
  ...
  function bindAccountForm(form) {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        const data = await apiJson(`${API_BASE}/api/auth/register`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json"
          },
        ...
        applyAuthState();
        announce("Compte créé avec succès. Vous êtes maintenant connecté.");
        form.reset();
        closeModal();
        setTimeout(() => showAccountPanel(), 150);
      } catch (error) {
        errorBox.textContent = error.message || "Impossible de créer le compte pour le moment.";
        errorBox.hidden = false;
        announce(error.message || "Impossible de créer le compte pour le moment.");
      }
    });
  }
  ...
  function moveMobileSettings() {
    ...
    if (mobile) {
      ...
      if (btnAccount.parentElement !== mainNav) {
        mainNav.insertBefore(btnAccount, mainNav.querySelector(".mobile-settings") || null);
      }
    } else {
      ...
      if (btnAccount.parentElement === mainNav) {
        headerActions.insertBefore(btnAccount, headerActions.lastElementChild);
      }
    }
  }
